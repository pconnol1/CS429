...
class index:
	...
	def and_query(self, query):
		...
		for i, term in enumerate(query):
			if i > 0:
				...
				result = self.mergeList(result, self.collection[term].keys())
			else:
	...
	@staticmethod
    #function to combine result lists for query
...
#initialize collection to be in the the file location ./collection from the home directory
...
# sample queries
obj.and_query(["yemen"])
obj.and_query(["with", "yemen"])
obj.and_query(["with", "without", "yemen"])
obj.and_query(["with", "without", "yemen", "citizens"])
obj.and_query(["with", "without", "yemen", "yemini", "citizens"])
