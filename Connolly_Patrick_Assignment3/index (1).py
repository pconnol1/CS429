...
class index:
	...
	#Return the new query  terms and their weights
		...
		if len(pos_feedback) > 0:
			...
			for doc in pos_feedback:
				for term, weight in self.document_vectors[doc - 1].items():
					query_terms[term] += beta * weight / len(pos_feedback)
		...
		if len(neg_feedback) > 0:
			...
			for doc in neg_feedback:
				for term, weight in self.document_vectors[doc - 1].items():
					query_terms[term] -= gamma * weight / len(neg_feedback)
	...
	def init_query_vector(self, query):
		query_init_dict = self.clean_query(query)
		query_vector = dict.fromkeys(self.all_tokens_set, 0)
	...
	def run_rocchio(self, query, k_docs):
		...
		while again == "y":
			...
			query_vector = self.rocchio(query_vector, rel_docs, non_rel_docs, 1.0, 0.75, 0.15)
